...
def cfg():
    ...
    c = defaults.copy()
    c.update(getattr(settings, "CROWD_METER", {}))
    return c
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Order model '{C['ORDER_MODEL']}' not found."))
            return
        ...
        status_counts = df[sf].value_counts()
        ...
        cap = C["CAPACITY"]
        tmp = df.copy()
        tmp["w"] = tmp[sf].map(weights).fillna(0.0)
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Saved ML model → {model_path} | MAE vs heuristic: {mae:.2f} | rows={len(data)}"
        ))
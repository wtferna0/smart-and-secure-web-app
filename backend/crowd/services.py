...
def _cfg():
    d = dict(
        WINDOW_MINUTES=30, ACTIVE_STATUSES=["Accepted","Preparing","Ready"],
        STATUS_WEIGHTS={"Accepted":1.0,"Preparing":0.8,"Ready":0.4},
        PEOPLE_PER_ORDER=1.3, CAPACITY=50, SMOOTHING_ALPHA=0.35,
...
def current_level():
    """
    1) If a manual override is active -> return it.
    2) Else try ML prediction; if unavailable -> heuristic.
    3) Smooth before returning.
    """
    now = timezone.now()
